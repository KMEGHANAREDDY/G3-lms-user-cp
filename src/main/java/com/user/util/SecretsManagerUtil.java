...
public class SecretsManagerUtil {
	private static final String SECRET_NAME = "G3-rds-secret";
	...
	public static String getRDSPassword() {
		try (SecretsManagerClient client = SecretsManagerClient.builder().region(REGION).build()) {
			GetSecretValueRequest getSecretValueRequest = GetSecretValueRequest.builder().secretId(SECRET_NAME).build();
